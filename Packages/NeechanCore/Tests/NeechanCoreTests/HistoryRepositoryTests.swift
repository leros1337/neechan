...
struct HistoryRepositoryTests {
    ...
    func recordsVisit() async throws {
        ...
        try await repository.recordVisit(
            ThreadKey(site: .dvach, board: "b", threadNum: 1), title: "Тред", thumbnailPath: "/b/thumb/1/1s.jpg"
        )

        let entries = try await repository.recent(site: .dvach, limit: 10)
        #expect(entries.count == 1)
        #expect(entries.first?.key == ThreadKey(site: .dvach, board: "b", threadNum: 1))
    ...
    func ordersByRecency() async throws {
        let repository = try makeRepository()
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: 1), title: "Первый")
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: 2), title: "Второй")
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: 1), title: "Первый снова")
    ...
    func honoursLimit() async throws {
        let repository = try makeRepository()
        for num in 1...5 {
            try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: num), title: "#\(num)")
        }
        #expect(try await repository.recent(site: .dvach, limit: 3).count == 3)
    }
    ...
    func removesEntry() async throws {
        ...
        #expect(try await repository.recent(site: .dvach, limit: 10).isEmpty)
    }
    ...
    func boardIsPartOfIdentity() async throws {
        let repository = try makeRepository()
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: 1), title: "Б")
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "po", threadNum: 1), title: "По")
        #expect(try await repository.recent(site: .dvach, limit: 10).count == 2)
    }

    @Test("history can be searched by title")
    func searchesByTitle() async throws {
        let repository = try makeRepository()
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: 1), title: "Котики")
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: 2), title: "Собаки")

        let hits = try await repository.search("кот", site: .dvach, limit: 10)
        #expect(hits.count == 1)
    ...
    func respectsDisabledRecording() async throws {
        ...
        try await repository.recordVisit(ThreadKey(site: .dvach, board: "b", threadNum: 1), title: "Тред")
        #expect(try await repository.recent(site: .dvach, limit: 10).isEmpty)
    }
}
